...
BUSYBOX_CMDS := ls cat echo mkdir rm rmdir touch pwd true false cp mv stat chmod kill sleep time grep head tail wc sort uniq tr cut cmp ln readlink tee xargs find sed awk diff nl dirname basename env printenv timeout nohup tar gzip gunzip printf
...
clean:
	rm -rf $(BUILD_DIR) $(TARGET) $(DISASM_FILE) $(DISK_IMG) user/*.o user/*.elf
